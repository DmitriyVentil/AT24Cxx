...
void AT24C00_Write(uint8_t Dev_Adr, uint8_t Mem_Adr,uint8_t* data, uint8_t Data_Size)
{
	...
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data, (8-Page_count), 100)){};
	...
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data+8*i, 8, 100)){};
		Mem_Adr=Mem_Adr+8;
	}
	if(Data_Size%8!=0)
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data+8*(Data_Size/8), Data_Size%8, 100)){};
	}
}
/*
...
void AT24C00_Read(uint8_t Dev_Adr, uint8_t Mem_Adr,uint8_t* data, uint8_t Data_Size)
{
	...
	if((8-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data, (8-Page_count), 100)){};
		Data_Size=Data_Size-(8-Page_count);
		Mem_Adr=Mem_Adr+(8-Page_count);
		data=data+(8-Page_count);
	}
	for(uint8_t i=0;i<Data_Size/8;i++)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data+8*i, 8, 100)){};
		Mem_Adr=Mem_Adr+8;
	}
	if(Data_Size%8!=0)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr, 1, data+8*(Data_Size/8), Data_Size%8, 100)){};
	}
}
/*
...
uint8_t AT24C00_Search_Last(uint8_t Dev_Adr, uint8_t Mem_Adr_Start, uint8_t Mem_Adr_Stop)
{
	...
	if((8-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, data, (8-Page_count), 100)){};
		Data_Size=Data_Size-(8-Page_count);
	}
	...
	for(uint8_t i=0;i<Data_Size/8;i++)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, data, 8, 100)){};
		for(last_Adr_count=0;last_Adr_count<8;last_Adr_count++)
		{
			if(data[last_Adr_count]==0xFF)
			   return (Mem_Adr_Start+last_Adr_count);
	...
	if(Data_Size%8!=0)
	{
		while(HAL_I2C_Mem_Read(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, data, Data_Size%8, 100)){};
	}
	for(last_Adr_count=0;last_Adr_count<8;last_Adr_count++)
	{
		if(data[last_Adr_count]==0xFF)
...
void AT24C00_Erase(uint8_t Dev_Adr, uint8_t Mem_Adr_Start, uint8_t Mem_Adr_Stop)
{
	...
	if((8-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, (8-Page_count), 100)){};
		Data_Size=Data_Size-(8-Page_count);
		Mem_Adr_Start=Mem_Adr_Start+(8-Page_count);
	}
	...
	for(uint8_t i=0;i<Data_Size/8;i++)
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, 8, 100)){};
		Mem_Adr_Start=Mem_Adr_Start+8;
	}
	if(Data_Size%8!=0)
	{
		while(HAL_I2C_Mem_Write(AT24C00_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, Data_Size%8, 100)){};
	}
}
