...
void AT24C04_Write(uint8_t Dev_Adr, uint16_t Mem_Adr,uint8_t* data, uint16_t Data_Size)
{
	...
	if(Mem_Adr>0xFF){Dev_Adr=Dev_Adr | 0x02;}
	if((16-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data, (16-Page_count), 100)){};
	...
	{
		if(Mem_Adr>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data+16*i, 16, 100)){};
		Mem_Adr=Mem_Adr+16;
	}
	if(Data_Size%16!=0)
	{
		if(Mem_Adr>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data+16*(Data_Size/16), Data_Size%16, 100)){};
	}
}
/*
...
void AT24C04_Read(uint8_t Dev_Adr, uint16_t Mem_Adr,uint8_t* data, uint16_t Data_Size)
{
	...
	if((16-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data, (16-Page_count), 100)){};
		Data_Size=Data_Size-(16-Page_count);
		Mem_Adr=Mem_Adr+(16-Page_count);
		data=data+(16-Page_count);
	}
	...
	{
		if(Mem_Adr>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data+16*i, 16, 100)){};
		Mem_Adr=Mem_Adr+16;
	}
	if(Data_Size%16!=0)
	{
		if(Mem_Adr>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr, 1, data+16*(Data_Size/16), Data_Size%16, 100)){};
	}
}
/*
...
uint16_t AT24C04_Search_Last(uint8_t Dev_Adr, uint16_t Mem_Adr_Start, uint16_t Mem_Adr_Stop)
{
	...
	if(Mem_Adr_Start>0xFF){Dev_Adr=Dev_Adr | 0x02;}
	if((16-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, data, (16-Page_count), 100)){};
		Data_Size=Data_Size-(16-Page_count);
	}
	...
	{
		if(Mem_Adr_Start>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, data, 16, 100)){};
		for(last_Adr_count=0;last_Adr_count<16;last_Adr_count++)
			{
				if(data[last_Adr_count]==0xFF)
				   return (Mem_Adr_Start+last_Adr_count);
	...
	{
		if(Mem_Adr_Start>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Read(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, data, Data_Size%16, 100)){};
	}
	for(last_Adr_count=0;last_Adr_count<16;last_Adr_count++)
	{
		if(data[last_Adr_count]==0xFF)
...
void AT24C04_Erase(uint8_t Dev_Adr, uint16_t Mem_Adr_Start, uint16_t Mem_Adr_Stop)
{
	...
	if((16-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, (16-Page_count), 100)){};
		Data_Size=Data_Size-(16-Page_count);
		Mem_Adr_Start=Mem_Adr_Start+(16-Page_count);
	}
	...
	{
		if(Mem_Adr_Start>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, 16, 100)){};
		Mem_Adr_Start=Mem_Adr_Start+16;
	}
	if(Data_Size%16!=0)
	{
		if(Mem_Adr_Start>0xFF){Dev_Adr=Dev_Adr | 0x02;}
		while(HAL_I2C_Mem_Write(AT24C04_I2C, Dev_Adr, Mem_Adr_Start, 1, EraseBuff, Data_Size%16, 100)){};
	}
}
