...
void AT24CM01_Write(uint8_t Dev_Adr, uint32_t Mem_Adr,uint8_t* data, uint32_t Data_Size)
{
	...
	{
		while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data, (256-Page_count), 100)){};
	...
	{
		Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr>>15);
		while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data+256*i, 256, 100)){};
		Mem_Adr=Mem_Adr+256;
	}
	if(Data_Size%256!=0)
	{
		Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr>>15);
		while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data+256*(Data_Size/256), Data_Size%256, 100)){};
	}
}
/*
...
void AT24CM01_Read(uint8_t Dev_Adr, uint32_t Mem_Adr,uint8_t* data, uint32_t Data_Size)
{
	...
	if((256-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data,Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data, (256-Page_count), 100)){};
		Data_Size=Data_Size-(256-Page_count);
		Mem_Adr=Mem_Adr+(256-Page_count);
		data=data+(256-Page_count);
	}
	...
	{
		Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr>>15);
		while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data+256*i, 256, 100)){};
		Mem_Adr=Mem_Adr+256;
	}
	if(Data_Size%256!=0)
	{
		Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr>>15);
		while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr, 2, data+256*(Data_Size/256), Data_Size%256, 100)){};
	}
}
/*
...
uint32_t AT24CM01_Search_Last(uint8_t Dev_Adr, uint32_t Mem_Adr_Start, uint32_t Mem_Adr_Stop)
{
	if(Mem_Adr_Start<Mem_Adr_Stop)
	{
		...
		if((256-Page_count)>Data_Size)
		{
			while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, data, Data_Size, 100)){};
		}
		else
		{
			while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, data, (256-Page_count), 100)){};
			Data_Size=Data_Size-(256-Page_count);
		}
		...
		{
			Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr_Start>>15);
			while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, data, 256, 100)){};
			for(last_Adr_count=0;last_Adr_count<256;last_Adr_count++)
			{
				if(data[last_Adr_count]==0xFF)
				   return (Mem_Adr_Start+last_Adr_count);
		...
		{
			Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr_Start>>15);
			while(HAL_I2C_Mem_Read(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, data, Data_Size%256, 100)){};
		}
		for(last_Adr_count=0;last_Adr_count<256;last_Adr_count++)
		{
			if(data[last_Adr_count]==0xFF)
...
void AT24CM01_Erase(uint8_t Dev_Adr, uint32_t Mem_Adr_Start, uint32_t Mem_Adr_Stop)
{
	if(Mem_Adr_Start<Mem_Adr_Stop)
	{
		...
		if((256-Page_count)>Data_Size)
		{
			while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, EraseBuff, Data_Size, 100)){};
		}
		else
		{
			while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, EraseBuff, (256-Page_count), 100)){};
			Data_Size=Data_Size-(256-Page_count);
			Mem_Adr_Start=Mem_Adr_Start+(256-Page_count);
		}
		...
		{
			Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr_Start>>15);
			while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, EraseBuff, 256, 100)){};
			Mem_Adr_Start=Mem_Adr_Start+256;
		}
		if(Data_Size%256!=0)
		{
			Dev_Adr_t= Dev_Adr | (uint8_t)(Mem_Adr_Start>>15);
			while(HAL_I2C_Mem_Write(AT24CM01_I2C, Dev_Adr_t, Mem_Adr_Start, 2, EraseBuff, Data_Size%256, 100)){};
		}
	}
}
