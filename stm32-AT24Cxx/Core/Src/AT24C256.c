...
void AT24C256_Write(uint8_t Dev_Adr, uint16_t Mem_Adr,uint8_t* data, uint32_t Data_Size)
{
	...
	{
		while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data, Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data, (64-Page_count), 100)){};
	...
	{
		while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data+64*i, 64, 100)){};
		Mem_Adr=Mem_Adr+64;
	}
	if(Data_Size%64!=0)
	{
		while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data+64*(Data_Size/64), Data_Size%64, 100)){};
	}
}
/*
...
void AT24C256_Read(uint8_t Dev_Adr, uint16_t Mem_Adr,uint8_t* data, uint32_t Data_Size)
{
	...
	if((64-Page_count)>Data_Size)
	{
		while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data,Data_Size, 100)){};
	}
	else
	{
		while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data, (64-Page_count), 100)){};
		Data_Size=Data_Size-(64-Page_count);
		Mem_Adr=Mem_Adr+(64-Page_count);
		data=data+(64-Page_count);
	}
	for(uint16_t i=0;i<Data_Size/64;i++)
	{
		while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data+64*i, 64, 100)){};
		Mem_Adr=Mem_Adr+64;
	}
	if(Data_Size%64!=0)
	{
		while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr, 2, data+64*(Data_Size/64), Data_Size%64, 100)){};
	}
}
/*
...
uint32_t AT24C256_Search_Last(uint8_t Dev_Adr, uint16_t Mem_Adr_Start, uint16_t Mem_Adr_Stop)
{
	if(Mem_Adr_Start<Mem_Adr_Stop)
	{
		...
		if((64-Page_count)>Data_Size)
		{
			while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, data, Data_Size, 100)){};
		}
		else
		{
			while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, data, (64-Page_count), 100)){};
			Data_Size=Data_Size-(64-Page_count);
		}
		...
		for(uint16_t i=0;i<Data_Size/64;i++)
		{
			while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, data, 64, 100)){};
			for(last_Adr_count=0;last_Adr_count<64;last_Adr_count++)
			{
				if(data[last_Adr_count]==0xFF)
				   return (Mem_Adr_Start+last_Adr_count);
		...
		if(Data_Size%64!=0)
		{
			while(HAL_I2C_Mem_Read(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, data, Data_Size%64, 100)){};
		}
		for(last_Adr_count=0;last_Adr_count<64;last_Adr_count++)
		{
			if(data[last_Adr_count]==0xFF)
...
void AT24C256_Erase(uint8_t Dev_Adr, uint16_t Mem_Adr_Start, uint16_t Mem_Adr_Stop)
{
	if(Mem_Adr_Start<Mem_Adr_Stop)
	{
		...
		if((64-Page_count)>Data_Size)
		{
			while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, EraseBuff, Data_Size, 100)){};
		}
		else
		{
			while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, EraseBuff, (64-Page_count), 100)){};
			Data_Size=Data_Size-(64-Page_count);
			Mem_Adr_Start=Mem_Adr_Start+(64-Page_count);
		}
		...
		for(uint16_t i=0;i<Data_Size/64;i++)
		{
			while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, EraseBuff, 64, 100)){};
			Mem_Adr_Start=Mem_Adr_Start+64;
		}
		if(Data_Size%64!=0)
		{
			while(HAL_I2C_Mem_Write(AT24C256_I2C, Dev_Adr, Mem_Adr_Start, 2, EraseBuff, Data_Size%64, 100)){};
		}
	}
}
